...
#import "YSLScrollMenuView1.h"

static const CGFloat kYSLScrollMenuView1Width  = 90;
static const CGFloat kYSLScrollMenuView1Margin = 10;
static const CGFloat kYSLIndicatorHeight = 3;

@interface YSLScrollMenuView1 ()
...
@implementation YSLScrollMenuView1
...
- (void)setItemTitleArray:(NSArray *)itemTitleArray
{
    if (_itemTitleArray != itemTitleArray) {
        ...
        for (int i = 0; i < itemTitleArray.count; i++) {
            CGRect frame = CGRectMake(0, 0, kYSLScrollMenuView1Width, CGRectGetHeight(self.frame));
        ...
        _indicatorView.frame = CGRectMake(10, _scrollView.frame.size.height - kYSLIndicatorHeight, kYSLScrollMenuView1Width, kYSLIndicatorHeight);
...
- (void)setIndicatorViewFrameWithRatio:(CGFloat)ratio isNextItem:(BOOL)isNextItem toIndex:(NSInteger)toIndex
{
    ...
    if (isNextItem) {
        indicatorX = ((kYSLScrollMenuView1Margin + kYSLScrollMenuView1Width) * ratio ) + (toIndex * kYSLScrollMenuView1Width) + ((toIndex + 1) * kYSLScrollMenuView1Margin);
    } else {
        indicatorX =  ((kYSLScrollMenuView1Margin + kYSLScrollMenuView1Width) * (1 - ratio) ) + (toIndex * kYSLScrollMenuView1Width) + ((toIndex + 1) * kYSLScrollMenuView1Margin);
    }
    
    if (indicatorX < kYSLScrollMenuView1Margin || indicatorX > self.scrollView.contentSize.width - (kYSLScrollMenuView1Margin + kYSLScrollMenuView1Width)) {
        return;
    }
    _indicatorView.frame = CGRectMake(indicatorX, _scrollView.frame.size.height - kYSLIndicatorHeight, kYSLScrollMenuView1Width, kYSLIndicatorHeight);
